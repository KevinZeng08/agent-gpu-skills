...
to improve performance, or merging multiple send/receive point-to-point operations (see [Point-to-point communication](p2p.md#point-to-point)
section). All three usages can be combined together, with one exception: calls to [`ncclCommInitRank()`](../api/comms.md#c.ncclCommInitRank)
...
#### WARNING
We do not recommed using CUDA graph capture when managing multiple GPUs from one thread. In some cases `cudaGraphLaunch` may block, preventing the launch across all GPUs. See [Using NCCL with CUDA Graphs](cudagraph.md#using-nccl-with-cuda-graphs) for details.
...
* [`ncclGroupStart()`](../api/group.md#c.ncclGroupStart)
* [`ncclGroupEnd()`](../api/group.md#c.ncclGroupEnd)
...
* [`ncclCommInitRankConfig()`](../api/comms.md#c.ncclCommInitRankConfig)
