...
in parallel to independent communicators located on the same device with multiple threads (see [Using multiple NCCL communicators concurrently](communicators.md#multi-thread-concurrent-usage)).
If the child communicator shares the resources with the parent communicator (i.e., [ncclConfig_t](../api/types.md#ncclconfig) by splitShare), it is not
