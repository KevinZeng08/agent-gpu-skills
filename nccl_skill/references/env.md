...
Forces NCCL to use a specific network, for example to make sure NCCL uses an external plugin and doesn’t automatically fall back on the internal IB or Socket implementation. Setting this environment variable will override the `netName` configuration in all communicators (see [ncclConfig_t](api/types.md#ncclconfig)); if not set (undefined), the network module will be determined by the configuration; if not passing configuration, NCCL will automatically choose the best network module.
...
<a id="nccl-debug-timestamp-format"></a>
...
<a id="nccl-debug-timestamp-levels"></a>
...
For more explanation about NCCL policies, please see [NCCL Communicator CTA Policy Flags](api/flags.md#cta-policy-flags).
...
which can also be set programmatically using [ncclCommInitRankConfig](api/comms.md#ncclcomminitrankconfig). If
...
Use CUDA cuMem\* functions to allocate host memory in NCCL. See [Shared memory](troubleshooting/runtime_and_mpi_issues.md#cumem-host-allocations) for more information.
...
#### WARNING
Using multiple GPUs per process with CUDA graph capture may result in deadlocks under certain conditions. See [Using NCCL with CUDA Graphs](usage/cudagraph.md#using-nccl-with-cuda-graphs) for details.
...
<a id="nccl-graph-mixing-support"></a>
...
<a id="nccl-graph-stream-ordering"></a>
...
`graphUsageMode=2`; see [ncclConfig_t](api/types.md#ncclconfig)) is **not supported**. If stream
ordering is disabled for a communicator, **graph mixing must be off**—use
`graphUsageMode` `0` or `1` (and note that [NCCL_GRAPH_MIXING_SUPPORT](#nccl-graph-mixing-support)
...
to per-communicator `graphStreamOrdering` `0`.
...
`graphStreamOrdering` field in [ncclConfig_t](api/types.md#ncclconfig). When that
...
for a communicator—see `graphStreamOrdering` for details.
...
variable will override the `cgaClusterSize` configuration in all communicators (see [ncclConfig_t](api/types.md#ncclconfig)); if not set
...
[ncclConfig_t](api/types.md#ncclconfig)); if not set (undefined), maximum CTAs will be determined by
...
[ncclConfig_t](api/types.md#ncclconfig)); if not set (undefined), minimum CTAs will be determined by
the configuration; if not passing configuration, NCCL will automatically choose
the best value.
...
NCCL jobs during execution (see [RAS](troubleshooting/ras.md)).

#### Values accepted

Default is 1 (enabled); define and set to 0 to disable RAS.

<a id="env-nccl-ras-addr"></a>
...
<a id="nccl-launch-order-implicit"></a>
