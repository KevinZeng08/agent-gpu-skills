...
as such, it is enabled by default (but see [NCCL_RAS_ENABLE](../env.md#env-nccl-ras-enable)).
...
[NCCL_RAS_ADDR](../env.md#env-nccl-ras-addr)).  The `ncclras` binary client can be used to connect to that socket and query the RAS
...
more machine-friendly format, see [JSON Output](#ras-json) below.
